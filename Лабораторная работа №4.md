...
- TOTAL ERROR(OR) среднее c каждой эпохой уменьшается и достигиает значения 0 примерно к 4 эпохе. То есть 10 эпох для OR слишком много, персептрон просто запоминает тренировочные данные. Оптимальное количество эпох равно четырём для OR
...
### Построить визуальную модель работы перцептрона на сцене Unity.  
  ...
  ```
Значение в каждом условие test мы меняем в зависимости от того, для какой функции мы проводим тесты. В данном случаче test представлен для функции XOR
